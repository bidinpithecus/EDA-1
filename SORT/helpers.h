...
int partition(int v[], int head, int tail, int *iterations) {
	int last = v[tail];
	int i = (head - 1);
...
void pivotQuickSort(int v[], int head, int tail, int *iterations) {
	if (head < tail) {
		int middle = partition(v, head, tail, *(&iterations));
		pivotQuickSort(v, head, middle - 1, *(&iterations)); //antes do pivot
		pivotQuickSort(v, middle + 1, tail, *(&iterations)); //depois do pivot
	}
}
...
void countingSort(int v[], int n, int exp, int *iterations) {
